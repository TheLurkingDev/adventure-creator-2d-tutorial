...
namespace AC
{
	...
	public class ActionListEditorWindow : EditorWindow
	{
		...
		private void PanAndZoomWindow (Event e)
		{
			...
			else if (e.type == EventType.KeyDown)
			{
				...
				else if (e.keyCode == KeyCode.PageUp)
				{
					...
					{
						ScrollPosition -= new Vector2 (0f, position.height);
					}
					UseEvent (e);
				}
		...
		private void CalculateDragRectRelative ()
		{
			...
			float bottomOverspill = position.height + (ScrollPosition.y - dragRect.y - dragRect.height - 60f) * Zoom;
		...
		private void UpdateDrag (Vector2 delta, bool forceUpdate = false)
		{
			...
			if ((delta.x + Actions[actionDragging].nodeRect.x + Actions[actionDragging].nodeRect.width + dragRectRelative.x) > (CanvasWidth / zoomMin))
			{
				delta.x = 0f;
			}
			if ((delta.y + Actions[actionDragging].nodeRect.y + Actions[actionDragging].nodeRect.height + dragRectRelative.y) > ((position.height - 50) / zoomMin))
			{
				delta.y = 0f;
			}
				...
				if (Actions[i] != null && (Actions[i].isMarked || i == actionDragging))
				{
					...
					if (i == actionDragging)
					{
						...
						if (delta.y > 0f || forceUpdate)
						{
							...
							float topSpill = (bottomNodeEdge - ScrollPosition.y) * Zoom - position.height;
		...
		private void ApplyEdgePanning (Event e)
		{
			...
			if (e.type == EventType.MouseDown || e.type == EventType.MouseDrag)
			{
				...
				float topMargin = position.height * dragMargin;
				float bottomMargin = position.height * (1f - dragMargin);
		...
		private void DrawScrollbars (Event e)
		{
			...
			Vector2 maxScrollbarSize = new Vector2 (CanvasWidth - 2f * scrollbarMargin, position.height - 2f * scrollbarMargin - 40f);


			// Horizontal
			float xScrollSize = CanvasWidth / (CanvasWidth + scrollLimits.x);// / Zoom / Zoom);
			if (xScrollSize < 1f)
			{
				float xValue = ScrollPosition.x / scrollLimits.x * Zoom;
				float xFreedom = maxScrollbarSize.x - (xScrollSize * maxScrollbarSize.x);

				Vector2 horizontalLeft = new Vector2 (xValue * xFreedom + scrollbarMargin, position.height - 34);
				Vector2 horizontalSize = new Vector2 (xScrollSize * maxScrollbarSize.x, scrollbarWidth / 2f);

				Rect horizontalRect = new Rect (horizontalLeft, horizontalSize);
				GUI.Box (horizontalRect, string.Empty, CustomStyles.Toolbar);
				...
				else if (e.type == EventType.MouseDrag && e.button == 0)
				{
					if (dragMode == DragMode.ScrollbarHorizontal)
					{
						...
						//float newScrollPosition = ScrollPosition.x + e.delta.x / Zoom;
			...
			if (yScrollSize < 1f)
			{
				...
				Vector2 verticalTop = new Vector2 (CanvasWidth - scrollbarMargin, yValue * yFreedom + scrollbarMargin + scrollbarMargin);
				Vector2 verticalSize = new Vector2 (scrollbarWidth / 2f, yScrollSize * maxScrollbarSize.y);

				Rect verticalRect = new Rect (verticalTop, verticalSize);
				GUI.Box (verticalRect, string.Empty, CustomStyles.Toolbar);
				...
				else if (e.type == EventType.MouseDrag && e.button == 0)
				{
					...
					{
						float newVerticalTop = e.mousePosition.y - scrollBarOffset;
						float newYValue = (newVerticalTop - scrollbarMargin) / yFreedom;
						float newScrollPosition = newYValue / Zoom * scrollLimits.y;

						//float newScrollPosition = ScrollPosition.y + e.delta.y / Zoom;
		...
		private void DrawMarquee (bool isAsset, Event e)
		{
			...
			{
				if (e.mousePosition.y > 24 && e.mousePosition.y < position.height - 22)
				{
		...
		private void UpdateScrollLimits ()
		{
			...
			scrollLimits.y = Mathf.Max (0f, scrollLimits.y - (position.height - 50f));
		}
		...
		private void NodesGUI (bool isAsset, Event e)
		{
			...
			EditorZoomArea.Begin (zoom, new Rect (0, 24, CanvasWidth / zoom, position.height / zoom - 44));
			GUI.BeginGroup (new Rect (0, 0, CanvasWidth / zoom, position.height / zoom - 44));
			...
			{
				if (/*hasDraggedWire &&*/ e.mousePosition.x > 0f && e.mousePosition.x < CanvasWidth / Zoom && e.mousePosition.y > 0f && e.mousePosition.y < (position.height - 45f) / Zoom)
				{
		...
		private void FocusOnActions (bool onlyIfNotInView = false, bool onlyMarked = true)
		{
			if (NumActionsMarked > 0 || !onlyMarked)
			{
				...
				Vector2 relativeScale = new Vector2 ((maxCorner.x - minCorner.x) / CanvasWidth, (maxCorner.y - minCorner.y) / position.height);
				...
				if (onlyIfNotInView)
				{
					if ((minCorner.x < ScrollPosition.x) || 
						...
						(maxCorner.y > ScrollPosition.y + position.height))
					{
						// OK, move
					}
					else
		...
		private void FocusOnAction (int i)
		{
			...
			if (Actions.Count > i && i >= 0 && Actions[i] != null)
			{
				...
				ScrollPosition = new Vector2 (centre.x - CanvasWidth / 2f, centre.y - position.height / 2f);
				Repaint ();
			}
		}
		...
		private void PerformEmptyCallBack (string objString)
		{
			...
			else if (objString == "ViewAll")
			{
				...
				Vector2 relativeScale = new Vector2 (maxCorner.x / CanvasWidth, maxCorner.y / position.height);
				float largestScale = Mathf.Max (relativeScale.x, relativeScale.y);
				Zoom = 1f / largestScale;
			}
		...
		public Vector2 CentrePosition
		{
			get
			{
				return (scrollPosition + (new Vector2 (CanvasWidth, position.height) / 2f));
			}
		}
		...
		private void DrawGrid (float gridSpacing, float gridOpacity, Color gridColor)
		{
			...
			{
				float x = (gridSpacing * i) + newOffset.x;
		...
		private float CanvasWidth
		{
			get
			{
				if (showProperties)
				{
					return position.width - propertiesBoxWidth + 7f;
				}
