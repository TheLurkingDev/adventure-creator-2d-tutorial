	...
	public class SaveSystem : MonoBehaviour
	{
		...
		protected virtual Texture2D GetScreenshotTexture ()
		{
			if (Camera.main != null)
			{
				...
				screenshotTexture.ReadPixels (KickStarter.mainCamera.GetPlayableScreenArea (false), 0, 0);
		...
		public virtual int ScreenshotWidth
		{
			get
			{
				return (int) ((float) KickStarter.mainCamera.GetPlayableScreenArea (false).width * KickStarter.settingsManager.screenshotResolutionFactor);
			}
		}
		...
		public virtual int ScreenshotHeight
		{
			get
			{
				return (int) ((float) KickStarter.mainCamera.GetPlayableScreenArea (false).height * KickStarter.settingsManager.screenshotResolutionFactor);
			}
		}
					...
					foreach (PlayerData _data in saveData.playerData)
					{
						if (_data.playerID == ID)
						{
							...
							if (doInventory)
							{
								...
								KickStarter.runtimeObjectives.AssignPlayerObjectives (_data);
							}
		...
		public static List<GVar> UnloadVariablesData (string data, List<GVar> existingVars, bool fromOptions = false)
		{
			...
			{
				copiedVars.Add (new GVar (existingVar));
			}
